...
On **March 11, 2026**, I published a roadmap checkpoint for `k1s` at `roadmap-dev-fabric-0-20260311`.

That checkpoint was not a feature release. It was a decision point.

I have done enough planning, implementation, and validation around the fabric path that it no longer makes sense to describe this project as only a small Kubernetes-like app engine with a few interesting distributed-systems ideas at the edges. That is still part of the story, but it is no longer the whole story.
...
> *The substrate the proposed hybrid architecture will run on, the "cloud", will not simply be "app engine" and "compute".*
> *It is part of the hybrid architecture itself, a "subconscious", or more directly a cognitive layer of the fabric control plane.*
...
- one [AI Max+ 395-based node](https://www.amd.com/en/products/processors/laptop/ryzen/ai-300-series/amd-ryzen-ai-max-plus-395.html) as a unit
- one 4-node AI Max+ 395-based execution cell as the first repeatable fabric cell
...
Fourth, the roadmap now names the actual program shape in public: `H*` for control-plane HA, `F*` for platform substrate phases, and `D*` for deployment milestones (see the [k1s roadmap status](https://github.com/the-cm-collective/k1s/blob/dev/docs/roadmap/status.md)). The current branch work maps into `F0` and `D0`, which means the present job is still hardening and validation, not narrating my way into maturity I do not have yet.
...
AMD’s Ryzen AI Max+ 395 platforms are available with up to 128 GB of unified LPDDR5x memory, and some configurations support exposing up to 96 GB as graphics-addressable memory. For this project, that is not just a marketing bullet. It means one node can carry a mix of dense inference, memory-heavy services, broker or control roles, and later locality-aware substrate work without immediately collapsing back into a one-box, one-role architecture.
...
The control plane should still learn capabilities, not SKU names. Storage class, RNIC family, negotiated PCIe state, and management-versus-fabric role separation belong in typed facts. But until that substrate is mature, an AI Max+ 395-based reference node gives me a concrete public baseline that is unusually well suited to mixed inference, memory-heavy services, and controller-adjacent roles. Standard Ethernet remains the D0 correctness baseline. RoCE is later F4 acceleration, not a prerequisite for the argument.
...
> *Open, legible, operator-controlled infrastructure is one of the few practical counterweights to increasingly opaque and centralized AI platforms.*
> *If the next generation of adaptive systems is only possible through black-box managed stacks with concentrated control over identity, runtime, data movement, and governance, then many of the most important decisions about those systems will already be politically settled before the public even understands the architecture.*
...
If you build toward persistent memory, adaptive coordination, self-modeling, long-horizon planning, locality-first knowledge services, and later cognitive-fabric behavior, then I think it is reckless to assume that moral questions begin only after someone proves a threshold event called "sentience."

The safer posture, in my view, is precautionary.
...
This direction is not emerging in a vacuum: projects like Hyperon/DAS, Cisco’s Internet of Cognition, NVIDIA Dynamo, DeepMind’s world-model work, and emerging model-welfare efforts suggest the field is already fragmenting beyond monolithic transformer stacks. What still appears rare is trying to unify substrate design, heterogeneous backbones, and welfare-first doctrine in one operator-controlled system.
